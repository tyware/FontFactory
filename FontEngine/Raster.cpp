...
#define maxVer 10000 
    ...
    // store the edge from (x1, y1) to (x2, y2) in table
    ...
    void ScanlineFill(int maxHeight)
    {
        ...
        int maxH = min(maxHt, maxHeight);
        for (i = 0; i<maxH; i++)//4. Increment y by 1 (next scan line) 
        {
            ...
            while (j<ActiveEdgeTuple.countEdgeBucket)
            {
                ...
                else
                {
                    ...
                    if (FillFlag)
                    {
                        // line from point(x1, i) to point(x2, i)
