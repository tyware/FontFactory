...
namespace FF_Font_Factory
{
    ...
    namespace FF_Font_Factory_TrueType
    {
        ...
        void CTablePost::Load(void* ptr, Long offset)
        {
            CTrueTypeFont *pFont = (CTrueTypeFont*) ptr;

            pFont->Seek(offset, SEEK_SET);

            format = pFont->ReadFixed();
            italicAngle = pFont->ReadFixed();
            underlinePosition = pFont->ReadFWord();
            underlineThickness = pFont->ReadFWord();
            isFixedPitch = pFont->ReadULong();
            minMemType42 = pFont->ReadULong();
            maxMemType42 = pFont->ReadULong();
            minMemType1 = pFont->ReadULong();
            maxMemType1 = pFont->ReadULong();
        }
